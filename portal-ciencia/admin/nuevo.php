...
// Lógica para crear nueva noticia
?><?php
session_start();
include("auth.php");
...
if (isset($_POST['importar_epmc']) && !empty($_POST['doi'])) {
  ...
  if (!empty($data['resultList']['result'][0])) {
    ...
    $titulo = $paper['title'] ?? '';
    $subtitulo = "Autores: " . ($paper['authorString'] ?? '');
    $contenido = $paper['abstractText'] ?? 'Resumen no disponible.';
    $doi = $_POST['doi'];
    $enlace_paper = $paper['fullTextUrlList']['fullTextUrl'][0]['url'] ?? '';
  } else {
...
if (isset($_POST['importar_ss']) && !empty($_POST['doi'])) {
  ...
  if (!empty($data['title'])) {
    ...
    $contenido = $data['abstract'] ?? 'Resumen no disponible.';
    $doi = $_POST['doi'];
    $enlace_paper = $data['url'] ?? '';
  } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['guardar'])) {
  ...
  $stmt = $conn->prepare("INSERT INTO noticias (titulo, subtitulo, contenido, categoria, imagen, fecha, doi, enlace_paper) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
  $stmt->bind_param("ssssssss", $titulo, $subtitulo, $contenido, $categoria, $imagen, $fecha, $doi, $enlace_paper);
  $stmt->execute();
...
<head>
  <meta charset="UTF-8">
  <title>Nueva noticia – Ciencia Hoy</title>
...
<body>
  <header>
    ...
    <p>Importa desde DOI o escribe manualmente</p>
  </header>
  ...
  <form method="POST" enctype="multipart/form-data">
    ...
    <button type="submit" name="guardar">Guardar noticia</button>
  </form>
</body>
</html>
